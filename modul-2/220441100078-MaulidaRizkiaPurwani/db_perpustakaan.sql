...
-- Anggota pinjam buku >5
CREATE VIEW v_pinjam_buku_terbanyak AS
SELECT a.id_anggota, a.nama_member, COUNT(b.kode_peminjaman) AS jumlah_pinjam
FROM tbl_anggota a
JOIN tbl_peminjaman b ON a.id_anggota = b.id_anggota
GROUP BY a.id_anggota  
HAVING COUNT(b.kode_peminjaman) > 5;
SELECT * FROM v_pinjam_buku_terbanyak;
-- drop view v_pinjam_buku_terbanyak;

-- Petugas yang melakukan transaksi beserta jumlah buku/jumlah total buku yg terlibat transaksi
CREATE VIEW v_transaksi_petugas AS
SELECT p.id_petugas, p.nama, COUNT(tp.kode_peminjaman) AS jumlah_transaksi
FROM tbl_petugas p LEFT JOIN tbl_peminjaman tp ON p.id_petugas = tp.id_petugas  
GROUP BY p.id_petugas;
SELECT * FROM v_transaksi_petugas;

-- 1 Petugas dengan transaksi terbanyak
CREATE VIEW v_transaksi_terbanyak AS
SELECT c.id_petugas, c.nama, COUNT(tt.kode_peminjaman) AS jumlah_transaksi
FROM tbl_petugas c LEFT JOIN tbl_peminjaman tt ON c.id_petugas = tt.id_petugas
GROUP BY c.id_petugas
ORDER BY COUNT(tt.kode_peminjaman) DESC  -- mengurutkan hasil dr terbanyak
LIMIT 1;
SELECT * FROM v_transaksi_terbanyak;

-- Buku terpinjam paling banyak
CREATE VIEW v_buku_terpinjam AS
SELECT b.kode_buku, b.judul, COUNT(p.kode_peminjaman) AS jumlah_peminjaman
FROM tbl_buku b LEFT JOIN tbl_peminjaman p ON b.kode_buku = p.kode_buku
GROUP BY b.kode_buku
ORDER BY COUNT(p.kode_peminjaman) DESC
LIMIT 1;
SELECT * FROM v_buku_terpinjam;

-- anggota yg paling banyak meminjam buku
CREATE VIEW v_anggota_terbanyak_meminjam AS
SELECT a.id_anggota, a.nama_member, COUNT(p.kode_peminjaman) AS jumlah_peminjaman
FROM tbl_anggota a
JOIN tbl_peminjaman p ON a.id_anggota = p.id_anggota
GROUP BY a.id_anggota, a.nama_member
ORDER BY COUNT(p.kode_peminjaman) DESC
LIMIT 1;
SELECT * FROM v_anggota_terbanyak_meminjam;

-- LEFT JOIN
SELECT p.kode_peminjaman, a.nama_member, b.judul
FROM tbl_peminjaman p
LEFT JOIN tbl_anggota a ON p.id_anggota = a.id_anggota
LEFT JOIN tbl_buku b ON p.kode_buku = b.kode_buku;

-- RIGHT JOIN
SELECT p.kode_peminjaman, a.nama_member, b.judul
FROM tbl_peminjaman p
RIGHT JOIN tbl_anggota a ON p.id_anggota = a.id_anggota
RIGHT JOIN tbl_buku b ON p.kode_buku = b.kode_buku;

-- inner join (menggabungkan baris dari beberapa tabel)
SELECT p.kode_peminjaman, a.nama_member, b.judul
FROM tbl_peminjaman p
INNER JOIN tbl_anggota a ON p.id_anggota = a.id_anggota
INNER JOIN tbl_buku b ON p.kode_buku = b.kode_buku;
...
-- Daftar buku terpinjam
SELECT b.kode_buku, b.judul, COUNT(p.kode_peminjaman) AS jumlah_terpinjam
FROM tbl_buku b
LEFT JOIN tbl_peminjaman p ON b.kode_buku = p.kode_buku
GROUP BY b.kode_buku, b.judul;

-- Anggota yg meminjam buku tertentu
SELECT a.id_anggota, a.nama_member
FROM tbl_anggota a
JOIN tbl_peminjaman p ON a.id_anggota = p.id_anggota
JOIN tbl_buku b ON p.kode_buku = b.kode_buku
WHERE b.judul = 'Rindu Ibu';

-- Update nama view
ALTER VIEW v_peminjam_terbanyak RENAME TO v_anggota_peminjam_terbanyak;

-- Menambahkan kolom pada view
ALTER VIEW v_peminjam_terbanyak AS
SELECT a.id_anggota, a.nama_member, a.jenis_kelamin, COUNT(b.kode_peminjaman) AS jumlah_pinjam
FROM tbl_anggota a
JOIN tbl_peminjaman b ON a.id_anggota = b.id_anggota
GROUP BY a.id_anggota
HAVING COUNT(b.kode_peminjaman) > 5;

-- Mengubah isi data view
CREATE OR REPLACE VIEW v_peminjam_terbanyak AS
SELECT a.id_anggota, a.nama_member, COUNT(b.kode_peminjaman) AS jumlah_pinjam
FROM tbl_anggota a
JOIN tbl_peminjaman b ON a.id_anggota = b.id_anggota
GROUP BY a.id_anggota
HAVING COUNT(b.kode_peminjaman) > 10;
SELECT * FROM v_peminjam_terbanyak;
-- DROP VIEW v_peminjam_terbanyak;

-- Mengubah judul buku dengan kode_buku tertentu
UPDATE tbl_buku
SET judul = 'Judul Baru'
WHERE kode_buku = 'kode_buku_tertentu';

SHOW TABLES;

-- drop database db_perpustakaan;
 -- DROP VIEW IF EXISTS v_peminjam_terbanyak;
 -- DROP VIEW IF EXISTS v_transaksi_terbanyak;
 -- DROP VIEW IF EXISTS v_transaksi_petugas;
 -- DROP VIEW IF EXISTS v_buku_terpinjam;
